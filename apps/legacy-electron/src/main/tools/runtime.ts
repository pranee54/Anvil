...
import type { AppSettings, FileChange, ToolCall, ToolResult } from '@shared/types'
...
export class ToolRuntime {
  ...
  private async inspectProject(): Promise<string> {
    const summary = await this.context.buildSummary()
    const treeSample = summary.files.slice(0, 80).join('\n')
    return [
      `Project root: ${summary.root}`,
      `Detected type: ${summary.type}`,
      `Important files:\n${summary.importantFiles.map((f) => `- ${f}`).join('\n') || '(none)'}`,
      summary.agentsMd ? `AGENTS.md instructions:\n${summary.agentsMd}` : 'No AGENTS.md found.',
      `File sample (${Math.min(80, summary.files.length)} of ${summary.files.length}):\n${treeSample}`
    ].join('\n\n')
  }
