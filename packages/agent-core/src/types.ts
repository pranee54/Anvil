...
export interface AgentActivity {
  id: string
  type: 'plan' | 'tool' | 'result' | 'error' | 'status' | 'permission' | 'stream'
  message: string
  toolName?: string
  timestamp: number
...
/** Structured tool activity for UI cards (never chain-of-thought). */
export interface ToolActivityCard {
  id: string
  category: 'search' | 'read' | 'edit' | 'terminal' | 'git' | 'result' | 'error' | 'status' | 'plan'
  title: string
  detail?: string
  path?: string
  command?: string
...
export interface AgentRunRequest {
  ...
  /** Prior turns in this chat (for follow-ups). Excludes the current message. */
...
/** Optional IDE hooks provided by the host (VS Code extension). */
