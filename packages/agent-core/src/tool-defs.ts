import type { PermissionLevel, ToolDefinition } from './types'
...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      'Get current IDE diagnostics/problems (TypeScript, Dart analyzer, ESLint, etc.) with file, line, severity, and message.',
    permission: 'safe',
    parameters: {
      type: 'object',
      properties: {
        ...
        severity: {
          type: 'string',
          description: 'Optional minimum severity: error | warning | info'
        }
      }
    }
  }
...
export function toolsForMode(mode: 'ask' | 'edit' | 'agent' | 'plan'): ToolDefinition[] {
