...
export class ContextEngine {
  ...
  async detectProjectType(): Promise<ProjectType> {
    ...
    // PHP / browser-extension backends (e.g. XAMPP apps without composer/artisan)
    if (
      (await has('backend')) ||
      (await has('extension')) ||
      (await has('extension/chrome/manifest.json')) ||
      (await has('extension/firefox/manifest.json'))
    ) {
      const phpish = await fg(['**/*.php', '**/manifest.json'], {
        cwd: root,
        onlyFiles: true,
        absolute: false,
        suppressErrors: true,
  ...
  classifyIntent(userMessage: string): ContextIntent {
    ...
    if (
      (/\b(how (does|do|is)|what (is|does)|explain|overview|architecture|works?|understand|ela|avutadhi|chudhu)\b/.test(
        lower
      ) ||
        /project.*(work|do)|work.*project/.test(lower)) &&
